...
#Properties
    ...
    def mid_strain_xy(self):
        ...
        if self._mid_strain_xy is None:
            load_array = sp.matrix(np.vstack((self.load.Nx,self.load.Ny,self.load.Nxy,self.load.Mx,self.load.My,self.load.Mxy)))
    ...
    def stress_xy(self):
        ...
        if self._stress_xy is None:    
            ...
            for q_bar in self.laminate.Q_layup:
                stress_xy_ply = q_bar*self.mid_strain_xy[0:3] + self.laminate.z_position[ply_num]*self.mid_strain_xy[3:7]
    ...
    def TW_i(self):
        ...
        if self._TW_i is None:
            ...
            for X in self.laminate.layup:
                ...
                TW_i = float(F1*self.stress_12[ply_num][0] + F2*self.stress_12[ply_num][1] + 
                        F11*self.stress_12[ply_num][0]**2 + F22*self.stress_12[ply_num][1]**2 +
                        F66*self.stress_12[ply_num][2]**2 + 2*F12*self.stress_12[ply_num][0]*self.stress_12[ply_num][1])
    ...
    def FPF(self):
        ...
        if self._FPF is None:
            ...
            while len(failed_plies) == 0:
                ...
                load_array = i*sp.matrix(np.vstack((self.load.Nx,self.load.Ny,self.load.Nxy,self.load.Mx,self.load.My,self.load.Mxy)))
                mid_strain_xy = ABD_p*load_array
                stress_xy_lam = []
                stress_12_lam = []
                TW_i_lam = []
                for x in range(len(self.laminate.layup)):
                    stress_xy_ply = self.laminate.Q_layup[x]*mid_strain_xy[0:3] + self.laminate.z_position[x]*mid_strain_xy[3:7]
                    stress_xy_lam.append(stress_xy_ply)
                    stress_12_ply = self.laminate.T_layup[x][0]*stress_xy_ply
                    stress_12_lam.append(stress_12_ply)

                    F1 = 1/self.laminate.layup[x][1].t1 + 1/self.laminate.layup[x][1].c1
                    F2 = 1/self.laminate.layup[x][1].t2 + 1/self.laminate.layup[x][1].c2
                    F11 = -1/(self.laminate.layup[x][1].t1*self.laminate.layup[x][1].c1)
                    F22 = -1/(self.laminate.layup[x][1].t2*self.laminate.layup[x][1].c2)
                    F66 = 1/self.laminate.layup[x][1].s**2
                    F12 = 1/(2*self.laminate.layup[x][1].t1*self.laminate.layup[x][1].c1) 
                    
                    TW_i = float(F1*stress_12_ply[0] + F2*stress_12_ply[1] + 
                            F11*stress_12_ply[0]**2 + F22*stress_12_ply[1]**2 +
                            F66*stress_12_ply[2]**2 + 2*F12*stress_12_ply[0]*stress_12_ply[1])
                    TW_i_lam.append(TW_i)
...
if __name__ == '__main__':
    ...
    analysis = Strength(laminate,load)
    print(analysis)
    print(analysis.TW_i)